...
const ContactDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const p_numberTemplate1 = (rowData, { rowIndex }) => <p >{rowData.teleNumber}</p>
...
<Column field="teleNumber" header="TeleNumber" body={p_numberTemplate1} filter={selectedFilterFields.includes("teleNumber")} hidden={selectedHideFields?.includes("teleNumber")}  sortable style={{ minWidth: "8rem" }} />
      ...
      {selectedItems.length > 0 ? (
        ...
        <Dialog header="Upload Contact Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="contact"            
          onUploadComplete={() => {
      ...
      <Dialog header="Search Contact" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
