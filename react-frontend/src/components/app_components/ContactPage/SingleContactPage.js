...
const SingleContactPage = (props) => {
  ...
  const [isHelpSidebarVisible, setHelpSidebarVisible] = useState(false);
    ...
    useEffect(() => {
        ...
        client
            .service("contact")
            .get(urlParams.singleContactId, { query: { $populate: [            {
                path: "createdBy",
                service: "users",
                select: ["name"],
              },{
    ...
    }, [props,urlParams.singleContactId]);
    ...
    return (
        ...
        <div className="col-12 flex flex-column align-items-center">
            <div className="col-12">
                ...
                {/* <p>contact/{urlParams.singleContactId}</p> */}
            </div>
            <div className="card w-full">
                <div className="grid ">

            <div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">TeleNumber</label><p className="m-0 ml-3" >{Number(_entity?.teleNumber)}</p></div>
            <div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Name</label>
                    {name.map((elem) => (
                        <Link key={elem._id} to={`/student/${elem._id}`}>
                        <div>
                  {" "}
      ...
      <CommentsSection
        recordId={urlParams.singleContactId}
        user={props.user}
        alert={props.alert}
        serviceName="contact"
      />
...
export default connect(mapState, mapDispatch)(SingleContactPage);
