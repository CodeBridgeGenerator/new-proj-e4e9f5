...
import contactFakerFactory from "./contactFakerFactory";
...
const ContactFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = contactFakerFactory(props.user,fakerCount,nameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("contact").create(elem));
...
export default connect(mapState, mapDispatch)(ContactFakerDialogComponent);
